...
private:
	uint16_t _next;
	uint8_t n();
	void n(uint8_t*, uint16_t len);
public:
  uint16_t length; // excluding M_Type byte
  M_Type type;
  uint8_t* data;
  uint8_t checksum; // type + data
  Message(uint16_t length_, M_Type type_): length(length_), type(type_), data(new uint8_t[length]), _next(0) {}
